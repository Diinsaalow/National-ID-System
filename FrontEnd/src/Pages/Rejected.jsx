...
function RejectedCases() {
    ...
    const loadAllRejected = async () => {
      try {
        const [births, ids] = await Promise.all([
          axios.get('http://localhost:5000/api/births/rejected'),
          axios.get('http://localhost:5000/api/ids/rejected'),
        ]);
  ...
  const handleSendEmailClick = (user) => {
    ...
    setMessage(`Dear ${user.fullName},\n\nWe regret to inform you that your application for a ${user.type} was rejected due to the following reason:\n\n${user.reason || 'Not specified'}\n\nYou may reapply or contact the office for more details.\n\nThank you,\nNIRA Team`);
    setShowModal(true);
  };
    ...
    <div className="max-w-6xl mx-auto p-6">
      ...
      <div className="bg-white shadow-xl rounded-xl overflow-x-auto">
        <table className="min-w-[800px] w-full text-sm">
          ...
          <tbody>
            {rejected.length === 0 ? (
              <tr><td colSpan="6" className="text-center py-6 text-gray-500">No rejected cases</td></tr>
            ) : rejected.map((r, i) => (
              <tr key={r._id} className="border-b hover:bg-gray-50">
                ...
                <td className="px-4 py-3 font-medium text-blue-800">{r.type || 'N/A'}</td>
                <td className="px-4 py-3 text-gray-700">{r.reason || 'N/A'}</td>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white w-full max-w-lg p-6 rounded-xl shadow-xl">
            <h3 className="text-xl font-bold text-red-600 mb-4">Send Rejection Email</h3>
            ...
            />
            <label className="block mt-4 text-sm font-medium mb-1">Subject</label>
            <input
              value={subject}
              onChange={(e) => setSubject(e.target.value)}
              className="w-full border p-2 rounded"
            />
            <label className="block mt-4 text-sm font-medium mb-1">Message</label>
            <textarea
              rows="6"
              value={message}
