...
const AllCitizenRecords = () => {
    ...
    const fetchData = async () => {
      try {
        const [idRes, birthRes] = await Promise.all([
          axios.get("http://localhost:5000/api/verified-ids"),
          axios.get("http://localhost:5000/api/verified-births"),
        ]);
  ...
  const headers = [
    "ID Number", "Full Name", "Date of Birth", "Gender", "Place of Birth",
    "Nationality", "Parent Serial", "Date of Expiry", "Photo",
    "Date of Issue", "County", "Email", "Type", "Actions"
  ];
            ...
            <thead className="bg-blue-100 text-blue-800">
              <tr>
                {headers.map((header, index) => (
                  <th key={index} className="px-4 py-3 font-semibold tracking-wide">
                    {header}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {currentRecords.map((citizen, index) => (
                <tr key={index} className="hover:bg-blue-50 border-b">
                  <td className="px-4 py-2">{citizen.idNumber || citizen.IDNumber}</td>
                  <td className="px-4 py-2">{citizen.fullName}</td>
                  <td className="px-4 py-2">{citizen.dob || citizen.dateOfBirth}</td>
                  ...
                  <td className="px-4 py-2">{citizen.parentSerial || citizen.parentSerialNumber}</td>
                  ...
                  <td className="px-4 py-2">
                    <Link to={`/view/${citizen._id}`} className="text-blue-600 hover:underline">
                      View
                    </Link>
                  </td>
                </tr>
