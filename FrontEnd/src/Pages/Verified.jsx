    ...
    const fetchVerified = async () => {
      try {
        const [birthRes, idRes] = await Promise.all([
          axios.get('http://localhost:5000/api/births?status=approved'),
          axios.get('http://localhost:5000/api/ids?status=approved'),
        ]);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="overflow-x-auto bg-white shadow-2xl rounded-xl">
        {loading ? (
          <div className="p-10 text-center text-gray-500 text-lg font-medium">⏳ Loading verified users...</div>
        ) : (
              ...
              {verifiedUsers.length === 0 ? (
                ...
                verifiedUsers.map((user, index) => (
                  <tr key={user._id} className="hover:bg-gray-50 border-t">
                    ...
                    <td className="px-4 py-3 text-blue-700 font-medium">{user.type || 'N/A'}</td>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white w-full max-w-lg p-6 rounded-xl shadow-xl">
            <h3 className="text-xl font-bold text-green-600 mb-4">Send Congratulations Email</h3>
