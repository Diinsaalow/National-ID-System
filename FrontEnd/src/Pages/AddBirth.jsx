...
import {
  FaBaby, FaUser, FaCalendar, FaTransgender, FaMapMarked, FaFlag, FaUserTie,
  FaCalendarAlt, FaGlobeAfrica, FaEnvelope, FaImage, FaDownload
} from 'react-icons/fa';
...
export default function AddBirth() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post('http://localhost:5000/api/births', data, {
  ...
  const inputStyle = 'p-3 border border-gray-300 rounded-md w-full focus:outline-none focus:ring-2 focus:ring-green-500 bg-white';
  ...
  const fields = [
    ...
    { name: 'gender', type: 'select', options: ['Male', 'Female'], icon: <FaTransgender /> },
    { name: 'placeOfBirth', placeholder: 'e.g. Beledweyne', icon: <FaMapMarked /> },
    { name: 'nationality', placeholder: 'e.g. Somali', icon: <FaFlag /> },
    { name: 'parentSerialNumber', placeholder: 'Enter Parent Serial No.', icon: <FaUserTie /> },
  ...
  return (
    <div className="flex flex-col lg:flex-row w-full min-h-screen bg-gray-100 font-sans">
      ...
      {/* RIGHT: Form Section */}
      <div className="w-full lg:w-1/2 h-full overflow-y-auto bg-white p-4 sm:p-6 lg:p-8" ref={printRef}>
        <div className="max-w-2xl mx-auto">
          ...
          <div className="bg-white border border-gray-200 rounded-lg shadow-lg p-4 sm:p-6">
            <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-4">
              {fields.map(({ name, type = 'text', placeholder, options, icon }) => (
                <div key={name}>
              ...
              <div className="flex flex-col sm:flex-row justify-center gap-4 pt-4">
                ...
                <button type="button" onClick={handleDownloadPDF} className="w-full sm:w-auto bg-blue-600 text-white px-6 py-2 rounded-md shadow hover:bg-blue-700 transition flex items-center justify-center gap-2">
          ...
          <div className="mt-8 bg-white border border-gray-300 rounded-md shadow p-4 sm:p-6 w-full mx-auto relative text-[11px] font-mono">
            <div className="text-center font-bold text-blue-900 text-lg uppercase mb-2">Republic of Somalia</div>
            <div className="text-center text-xs text-gray-700 uppercase mb-4">National Identity Card</div>
            <div className="flex pt-4 gap-4 flex-col lg:flex-row">
              ...
              <div className="w-[100px] h-[120px] border border-gray-400 bg-gray-100 rounded-md flex items-center justify-center overflow-hidden">
                {photoPreview ? (
                  ...
                  <span className="text-gray-400 text-center text-xs italic">No Photo</span>
                )}
              </div>
            </div>
