...
router.post('/', upload.single('photoFile'), async (req, res) => {
  try {
    ...
    await newID.save();
    res.status(201).json({ message: '✅ ID Card submitted successfully', id: newID._id });
  } catch (error) {
    console.error('❌ Error submitting ID:', error);
    res.status(500).json({ message: 'Failed to submit ID card', error: error.message });
  }
});
...
router.patch('/:id/status', async (req, res) => {
  ...
  try {
    ...
    const updated = await IDCard.findByIdAndUpdate(req.params.id, update, { new: true });
    if (!updated) return res.status(404).json({ message: 'ID record not found' });
...
router.get('/', async (req, res) => {
  try {
    ...
    res.status(500).json({ message: '❌ Failed to fetch ID cards', error: error.message });
  }
});
...
router.get('/pending', async (req, res) => {
  try {
    ...
    res.status(500).json({ message: '❌ Failed to load pending IDs', error: error.message });
  }
});
...
router.get('/rejected', async (req, res) => {
  try {
    ...
    res.status(500).json({ message: '❌ Failed to load rejected IDs', error: error.message });
  }
});
...
router.get('/verified', async (req, res) => {
  try {
    ...
    res.status(500).json({ message: '❌ Failed to load verified IDs', error: error.message });
  }
});
