...
router.patch('/:id/status', async (req, res) => {
  ...
  try {
    ...
    const updatedRecord = await Birth.findByIdAndUpdate(req.params.id, update, { new: true });
    if (!updatedRecord) {
...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ message: '❌ Failed to fetch birth records', error: error.message });
  }
});
